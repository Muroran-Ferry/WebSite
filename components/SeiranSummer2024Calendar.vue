<template>
  <div class="not-prose px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h5 class="text-base font-semibold leading-6 text-gray-900">
          夏期臨時便運航について
        </h5>
        <p class="mt-2 text-sm text-gray-700">
          2024年8月11日, 8月18日（日）2便（20:00）と 8月12日,
          8月19日（月）1便（09:00）を臨時運航いたします。
        </p>
        <p class="mt-2 text-sm text-gray-700">
          2024年8月3日（土）～2024年8月20日（火）の運航については、次の通りとなります。
        </p>
      </div>
    </div>
    <div class="mt-0 flow-root">
      <div
        class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8"
        data-scroll-hint
      >
        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
          <table class="min-w-full divide-y divide-gray-300">
            <thead>
              <tr>
                <th
                  scope="col"
                  class="whitespace-nowrap py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-0"
                >
                  便 (発地)
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  出発時刻
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/3（土）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/4（日）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/5（月）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/6（火）〜8/10（土）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/11（日）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/12（月）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/13（火）〜8/17（土）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap px-3 py-3.5 text-center text-sm font-semibold text-gray-900"
                >
                  8/18（日）
                </th>
                <th
                  scope="col"
                  class="whitespace-nowrap py-3.5 pl-3 pr-4 text-center text-sm font-semibold text-gray-900 sm:pr-0"
                >
                  8/19（月）
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
              <tr>
                <th
                  scope="row"
                  class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-0"
                >
                  1 (青森発)
                </th>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm font-medium text-gray-900"
                >
                  09:00
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  定休
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-orange-500"
                >
                  ○<br />
                  臨時運航
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm text-orange-500 sm:pr-0"
                >
                  ○<br />
                  臨時運航
                </td>
              </tr>
              <tr>
                <th
                  scope="row"
                  class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-0"
                >
                  2 (室蘭発)
                </th>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-900"
                >
                  20:00
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  定休
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-orange-500"
                >
                  ○<br />
                  臨時運航
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-gray-500"
                >
                  ○
                </td>
                <td
                  class="whitespace-nowrap px-3 py-4 text-center text-sm text-orange-500"
                >
                  ○<br />
                  臨時運航
                </td>
                <td
                  class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm text-gray-500 sm:pr-0"
                >
                  ○
                </td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>
    </div>
  </div>
</template>
